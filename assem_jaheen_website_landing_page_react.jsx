// index.tsx — Single‑file production‑ready landing page (no external deps)
// - Self‑contained React component (named + default export)
// - Arabic/English UI, sections (Hero, Services, Cases, Contact, Footer)
// - Contact form with graceful fallback if /api/contact is missing
// - Includes lightweight "tests" via console.assert (kept & extended)
// - No external libraries/imports besides React
...
/********************
 * Dev tests (do not remove existing; add more)
 ********************/
if (typeof window !== "undefined") {
  // Existing style test: ensure component is a function
  console.assert(typeof AssemJaheenSite === "function", "AssemJaheenSite component must be a function");

  // New tests
  console.assert(CONTACT_EMAILS.includes("assem@assemjaheen.com") && CONTACT_EMAILS.includes("info@assemjaheen.com"), "Business emails must be present");
  console.assert(/^[0-9]+$/.test(WHATSAPP_PHONE), "WhatsApp digits only for wa.me");
  try {
